...
func newInitCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			boundProfile := strings.TrimSpace(profileName)
			if cmd.Flags().Changed("profile") {
				if boundProfile == "" {
					return fmt.Errorf("init: --profile must not be empty")
				}
				if !steps.ValidCustomName(boundProfile) {
					return fmt.Errorf("init: invalid profile name %q (use lowercase letters, digits, '-' and '_', starting with a letter or digit)", boundProfile)
				}
			}
			...
			// Optional host-local profile binding: same trust level as the
			// global config (machine-owner-authored), overrides the repo
			// config's profile: field, and needs nothing committed to the repo.
			if boundProfile != "" {
				warnIfProfileUnusable(cmd.OutOrStdout(), p, boundProfile)
				if err := d.SetRepoLocalProfile(repo.ID, boundProfile); err != nil {
					return fmt.Errorf("init: set profile binding: %w", err)
				}
			}
			...
			if boundProfile != "" {
				fmt.Fprintf(w, "  %s  %s %s\n", sDim.Render("profile"), boundProfile, sDim.Render("(host-local binding)"))
			}
			if skillErr != nil {
	...
	cmd.Flags().StringVar(&profileName, "profile", "", "shared gate profile to bind this repo to (host-local; equivalent to 'no-mistakes profile use')")
	return cmd
}
