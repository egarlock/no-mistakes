...
func (d *DB) getRepos() ([]*Repo, error) {
	rows, err := d.sql.Query(`SELECT id, working_path, upstream_url, COALESCE(fork_url, ''), default_branch, COALESCE(local_profile, ''), created_at FROM repos ORDER BY working_path`)
	if err != nil {
	...
	for rows.Next() {
		repo := &Repo{}
		if err := rows.Scan(&repo.ID, &repo.WorkingPath, &repo.UpstreamURL, &repo.ForkURL, &repo.DefaultBranch, &repo.LocalProfile, &repo.CreatedAt); err != nil {
