...
- **Opinionated pipeline** - the default step sequence is `intent → rebase → review → test → document → lint → push → pr → ci`, and every repo gets all of it unless the repo's trusted default-branch config trims or reorders it with a [`steps:` list](/no-mistakes/reference/repo-config/#steps). You can also configure the commands each step runs, how many auto-fix attempts are allowed, and whether transcript-based intent extraction is used when intent is not supplied directly.
