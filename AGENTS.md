...
- Deferred (NOT PR-A): CLI (`no-mistakes profile list/show/lint`), phase-2 git-fetched sources + per-repo `profile: name@ref` pins, and name-keyed per-step override merge. Do not pull these forward. `mode: revise` from a shared profile (fleet-wide auto-commits) is an open design question — keep revise-mode profile steps out until the captain rules on the blast-radius posture.
