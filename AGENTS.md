...
- `ignore_patterns` is NOT code-executing but selects whether validation executes (a pushed `["*"]` matches every file by basename and turns every review-type gate — built-in review, repo skill reviews, profile-supplied skill reviews — into a vacuous pass), so `EffectiveRepoConfig` treats it like the selection fields: trusted default-branch copy only, pushed copy honored solely under `allow_repo_commands`, forced empty with no trusted copy. Regression: `TestEffectiveRepoConfig_IgnorePatternsTrustedOnly`.
- Non-executing fields (`auto_fix`, `intent`, `test`) are still read from the pushed branch.
...
**Skill-Driven Steps (`type: skill`, `mode: review` | `mode: revise`)**

- A `steps:` mapping carrying a `skill:` path is a **skill-driven step** (`config.StepSpec.IsSkill()` → `steps.SkillStep`, `internal/pipeline/steps/skillstep.go`). It is the SAME machinery as the built-in `review` step — a prompt template + a findings JSON schema handed to `sctx.Agent.Run` — but the prompt body comes from a repo-owned skill markdown file. Agent-agnostic by construction: the body is inlined into the prompt (no skill-invocation channel; works with codex/opencode). `config.StepSpec` grew `Type`, `Skill`, `Mode`, `RequireReview`, and the resolved `SkillBody` (all compared by `StepSpecsEqual`; `SkillBody` populated by the daemon — never parsed from YAML). `IsCommand()`/`IsSkill()` are mutually exclusive; `validateStepSpecs`/`validateCustomStepSpec`/`validateSkillStepSpec` enforce it plus: name pattern + no built-in collision (shared with command steps), a `type:` that agrees with the payload, a repo-relative non-escaping `skill` path, and `mode ∈ {"", "review", "revise"}`. `isCustomStepSpec` routes a `type: skill`/`command` spec with a missing payload to the custom validator so the error is precise, not "unknown step".
...
- **Read-only guard (enforced, not hoped)**: `mode: review` must not mutate the worktree **or its history**. `SkillStep.enforceReadOnly` snapshots `git status --porcelain` AND `git rev-parse HEAD` before the review agent runs and, if either changed after, discards the edits (`git reset --hard <snapshot HEAD>` + `git clean -fd`) and appends a warning finding ("skill modified the worktree during a review-mode step; changes were discarded") that gates the step. The HEAD comparison is load-bearing: an agent that edits then runs `git add -A && git commit` (agents habitually commit) leaves a CLEAN tree, so a porcelain-only check reports nothing and the rogue commit ships to the remote via the push step (which pushes `HEAD:<ref>` resolved from the worktree) with zero trace at the gate; a `git reset --hard <earlier>` likewise leaves a clean tree and would force-push truncated history under a still-valid lease. Resetting to the snapshot SHA covers commit, amend, and reset-back in one check. The guard runs on the read-only review pass (including the post-fix confirmation review), NOT inside a fix round's mutation phase (a user `fix` round is allowed to edit and commits via `commitAgentFixes`). Regressions: `TestSkillStep_WorktreeGuard_ResetsAndWarns`, `TestSkillStep_ReadOnlyGuard_AgentCommitBypass`, `TestSkillStep_ReadOnlyGuard_AgentResetBack`.
- **`mode: revise` (revision + review, mutating)**: `SkillStep.runRevise` (the non-fixing pass when `Mode == SkillModeRevise`) runs a mutation-permitting prompt (`revisePrompt`: "You MAY edit files: apply safe revisions directly, then re-review your own result", `findingsSchema`, `summary` field doubles as the commit subject) — NO read-only guard. It then commits whatever changed via **`commitAgentFixes` — the SAME commit protocol** `document`/`lint`/fix-rounds use, which advances `sctx.Run.HeadSHA`, persists it (`DB.UpdateRunHeadSHA`), and fast-forwards the local branch ref (`git update-ref`). **Force-push safety composes for free precisely because this protocol is reused**: the revise commit is additional local history built on the run base, so the push step's lease (`resolveForcePushDecision`, anchored to `lastFetchedBranchTip` + `Run.BaseSHA`) still refuses to clobber unseen upstream commits — do NOT hand-roll a different commit path here or you risk breaking that. A fix round (`sctx.Fixing`) never enters `runRevise`; it goes through `runFix` then the read-only confirmation review, exactly like review mode. **Self-commit rewind**: `runRevise` snapshots the worktree HEAD before the agent pass and, if the agent committed on its own (clean tree → `commitAgentFixes` would no-op, `mutated` would read false, `require_review` would be evaded, `Run.HeadSHA`/branch ref would go stale while the mutation still reached the remote at push), it runs `git reset --soft <snapshot>` so the changes land back in the tree and are committed through the one real protocol; a failed rewind fails the step (never push unbookkept history). The revise/fix prompts also tell the agent NOT to run `git commit`. `require_review: true` (`StepSpec.RequireReview` → `SkillStep.RequireReview`) forces `NeedsApproval` and appends an ask-user finding pointing at the committed diff range **only when a mutation actually happened** (`commitAgentFixes` is a no-op on a clean tree, so a no-change revise never force-parks). Regressions: `TestSkillStep_Revise_CommitProtocol` (HeadSHA + DB + branch ref), `TestSkillStep_Revise_RequireReviewParks`, `TestSkillStep_Revise_AgentSelfCommit` (self-commit rewound, protocol bookkeeping intact, require_review still parks), `TestSkillStep_Revise_NoMutationNoForcedPark`, `TestReviseStepThenPush_PreservesForcePushLease` (lease intact after a revise commit), e2e `TestSkillStepReviseFullRun` (revise between review and lint mutates, commits, run completes, commit reaches origin).
- **Ordering (hard error, not a warning)**: a `mode: revise` step MUST be listed before `push` — `validateStepSpecs` errors if any revise-mode skill index > push index ("mode: revise ... must come before push"). It mutates and commits, so after `push` its commits never reach the remote and a mutating commit would land after the force-push lease is anchored. This is stricter than the built-in mutating steps (review/test/document/lint after push are only a `stepChainProblems` *warning*). Regression: `TestBuildPipeline_ReviseMustPrecedePush`.
- Gate flow is identical to built-in review (`TestSkillStepGateFlow` e2e): findings park; `axi respond --action fix --findings <id> --instructions "..."` weaves the user's guidance (as finding `UserInstructions`, via `sanitizedPreviousFindingsForPrompt`) into the fix-round prompt. The e2e harness gained `SetupOpts.RepoExtraFiles` to commit a trusted skill body to the default branch (plus `Harness.OriginBranchLog`/`OriginBranchHasFile` for asserting pushed history). First-party templates ship at `.no-mistakes/skills/review.md` (read-only) and `.no-mistakes/skills/revise.md` (revision + review).

**Shared Gate Profiles (`profile:` field, `<NM_HOME>/profiles/<name>/`)**

- A shared gate profile is a host-local pipeline (steps + skills + instructions) defined once under `<NM_HOME>/profiles/<name>/` (via `paths.ProfileDir`/`paths.ProfileFile`) and selected by a repo's `profile: <name>` field, so one definition applies across many repos without per-repo copy-paste. `profile.yaml` (`config.ProfileConfig`, `LoadProfileFromBytes`) carries only `version` (informational, stamped) + a `steps:` list in the exact `StepSpec` schema repos use. PR-A ships phase 1 (host-local dir); a git-fetched source registry is a later PR. Docs: `docs/src/content/docs/guides/shared-profiles.md` (sidebar in `docs/astro.config.mjs`).
- **Trust (SECURITY, the core property):** `RepoConfig.Profile` is a **code-executing selection field** (it decides which shell commands and agent prompts run), so `EffectiveRepoConfig` takes it **only from the trusted default-branch copy** — a pushed branch can never set, switch, or drop it. Unlike `commands`/`agent`/`steps`, `profile` stays trusted-only **even under `allow_repo_commands`** (the safer v1 default; set deliberately in `EffectiveRepoConfig` before the opt-in early return). Profile *content* is never read from the worktree at all — it's host-local disk under a path no pushed commit can address. Regression: `TestEffectiveRepoConfig_ProfileTrustedOnly` (pushed cannot set/switch/remove, incl. under opt-in) + `TestSharedProfileAcrossTwoRepos` e2e.
- **Resolution & fail-closed:** the daemon (`RunManager.loadProfile`, `manager.go`) validates the name via `steps.ValidCustomName` (the exported `customStepNamePattern` reuse), reads `<NM_HOME>/profiles/<name>/profile.yaml`, and **fails the run at start** on missing/unparsable profile, bad name, unknown keys (`LoadProfileFromBytes` parses with `KnownFields(true)` — a typo'd `step:` must not parse to zero steps), zero steps (an empty list would flow through `ComposeProfileSteps` into `BuildPipeline`'s "empty = default pipeline" branch and silently replace the team gate), or a `mode: revise` skill step (fleet-wide auto-commits are a deliberately deferred design decision; the check must live at profile load because the post-`ComposeProfileSteps` merged list no longer knows which steps came from the profile) — never a silent fall back to the default pipeline (mirrors a bad `steps:` config). A selected profile must also be *verifiable*: when the default-branch fetch/resolve fails (empty trusted SHA) and the pushed config names a profile, `startRun` fails the run (`unverifiedProfileError`) instead of silently running the default pipeline — unlike `commands`/`agent`/`steps`, where "force empty" is a safe fallback, dropping a profile drops the team gate. Regression: `TestLoadProfile_*`, `TestUnverifiedProfileError` (unit) + `TestProfileMissingFailsRun` (e2e).
- **Composition v1 (`config.ComposeProfileSteps`, two rules):** repo has no `steps:` → the profile's list is the pipeline; repo has `steps:` → it must contain exactly one `- use: profile` splice sentinel (`StepSpec.Use`/`IsProfileSplice`, `ProfileSpliceSentinel`), which expands in place; a repo `steps:` with a profile selected but no sentinel is an error. `config.HasProfileSplice` guards the no-profile case (a stray sentinel without a profile fails the run). The merged list runs through the existing `validateStepSpecs`. Regression: `TestComposeProfileSteps`, `TestProfileSpliceComposition` (e2e).
- **Profile-relative skill/instruction loading:** `loadProfileSkillBodies` / `loadProfileStepInstructions` (`manager.go`, disk-read siblings of `loadTrustedSkillBodies`/`loadTrustedStepInstructions`) resolve a profile step's `skill:`/`instructions:` paths **against the profile dir** (not the worktree, not a git SHA), guarded by `profilePathWithinDir` (rejects absolute + escaping paths — defense in depth atop `validateStepSpecs`). A missing/escaping file → empty body → the skill step parks (misconfiguration finding), matching built-in skill steps. Repo-sourced steps in a spliced list keep their trusted-SHA resolution; instruction blocks are concatenated profile-first via `joinInstructionSections`. Regression: `TestLoadProfileSkillBodies_*`, `TestLoadProfileStepInstructions_*`, `TestProfilePathWithinDir`.
- **Run-record stamp:** `db.Run.Profile` (`runs.profile` column + migration; `db.SetRunProfile`; exposed on `ipc.RunInfo.Profile` via `runToInfo`) records `<name>@<ref>` (`profileStamp`: profile-dir `git rev-parse HEAD` when it's a repo, else `sha256:<hash>` of profile.yaml), so a consumer can confirm which profile revision enforced a gate.
- Deferred (NOT PR-A): CLI (`no-mistakes profile list/show/lint`), phase-2 git-fetched sources + per-repo `profile: name@ref` pins, and name-keyed per-step override merge. Do not pull these forward. `mode: revise` from a shared profile (fleet-wide auto-commits) is an open design question — `loadProfile` now **rejects** revise-mode profile steps (fail the run at start; `TestLoadProfile_RejectsReviseMode`) until the captain rules on the blast-radius posture; do not relax that without the ruling.
